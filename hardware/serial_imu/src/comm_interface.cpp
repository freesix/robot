...
unsigned char* FindString(unsigned char* Data, const unsigned int DataLen, unsigned char* StringVal, const unsigned int StringLen)
{
	...
	if(DataLen < 1 || StringLen < 1 || StringVal == 0 || Data == 0 || DataLen < StringLen)
		return 0;
	
	s = Data;
	p = StringVal;
	
	for(i = 0; i < (DataLen - StringLen + 1); i++)
	{
...
DisposeFlag FindDispose(unsigned char* Data, const unsigned int DataLen, unsigned char* StringVal,unsigned char** SAddr, unsigned int* OKLen)
{
	unsigned char* p = 0;
	unsigned char* e = 0;
	unsigned int StringLen = strlen((char*)StringVal);
	
	p = FindString(Data, DataLen, StringVal, StringLen);
	if(p != 0)
	{
		*SAddr = p;
		e = p + StringLen;
		*OKLen = e - *SAddr;
		return MIDDLE_RETURN;
	}
